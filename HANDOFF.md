...
| Dreams | ✅ | Meaningful sleep remixes real memories and preoccupations into persistent, shareable dream fragments. Sources become noun-like dream imagery — development milestones become the experience itself (words arriving, thoughts joining), never the “Reached …” milestone copy spliced in as a noun; absence durations, resolved moment titles and learned user facts convert the same way. Legacy dreams that already splice the malformed milestone construction are repaired once on load |
...
| Object handling | ✅ | One physical model: a tap uses a placed object, a drag repositions it, and dragging it onto the shelf target puts it away. Opening the shelf never changes what an object means on tap — activation always uses. No Use/Put away popup, no permanent toolbar; drag-to-tray and an explicit Put away control on each room object (shown with the shelf open) remain the put-away paths |
...
**Objects answer to a tap.** The Use / Put away popup is gone. `objectInput.ts` is a pure classifier: a tap on a placed object starts the canonical use flow, a drag repositions it (threshold 12 px, no long-press), and dragging it onto a shelf target that appears only during the drag puts it away. Opening the shelf never changes what an object means — live smoke confirmed the old tray-open tap flip (Use became Put away while the tray was up) could silently remove a care object; activation now always uses, and put-away is an explicit separate action: a drag onto the open tray/drag target, or the dedicated Put away control shown on each room object while the shelf is open (keyboard- and screen-reader-reachable, no permanent toolbar). Inventory drags still place at the release point; taps still auto-place.
...
- **Development stage IDs stay internal.** New milestone memories use the existing English stage copy instead of storing `reached first_words`. A shared stage-memory parser maps legacy IDs through `getDevelopmentLabel()` at Memory Book and dream-generation boundaries, so old saves are preserved without leaking snake_case into displayed memory or generated dream prose. Live production smoke of the release found one residual leak: a save that booted under an older version could already *store* a dream fragment embedding the raw id (plus its `dreamed:` memory and a creature-authored transcript quoting it), and stored prose bypassed the display boundaries. `migrateGameState` now repairs such stored prose once on load (`mapLegacyStageIdsInProse`, snake-case ids only — ordinary words are never rewritten): memories, dream fragments, creature-authored conversation messages and `conversation.lastCreatureMessage`. User-authored messages are the player's own words and are preserved byte-for-byte.
- **Dreams splice dream imagery, never milestone or record copy.** Production smoke of the release surfaced the next layer of the same bug: the dream generator mapped a milestone memory to its UI copy and inserted it as a noun, producing `Reached First words floated above the room breathing in the dark…`. `generateDreamAfterSleep` now converts a source memory into a noun-like dream image at the dream-memory boundary — a development milestone becomes the experience itself (`the arrival of the first words`, `thoughts beginning to join`, per stage, localized), absence durations become quiet-room imagery, resolved daily moments read as their title, and learned user facts become what the user means to the creature (`your name`, `the thing you like`). No milestone copy, database record or raw stage id can reach a fragment; sources, `sourceMemoryIds`, moods, interest effects and the dream cadence are unchanged. The missing-memory fallback is localized, and the Polish templates never make the inserted image the subject of an agreeing verb — quoted pairs take the non-personal plural and the reverse template is impersonal — so any image stays grammatical without an inflection engine. Stored dreams that already splice the malformed milestone construction (both the legacy raw-id form and the `Reached First words` form) are repaired once on load by `repairMalformedDreamProse` — only that recognisable construction inside dream fragments and `dreamed:` memories, plus creature speech that recognisably retells such a dream (`I had a strange dream: …` / `Miałem dziwny sen: …`). Ordinary creature speech and user-authored text are never rewritten and no other prose is paraphrased.
...
Focused deterministic coverage now exercises each contract above, including legacy stage prose through dreams and Memory Book display, stored legacy prose repair on load, milestone dreams rendered as natural imagery (never `Reached First words floated…`), raw-stage-id-free fragments, stored malformed dream repair on load with user-authored and ordinary creature conversation preserved, localized Polish fallbacks and agreement-free Polish dream templates for every stage image, the shelf-open tap regression, a new-database schema without the redundant store, recent-moment avoidance and fresh-seed diversity, and the completed/interrupted/autonomous bond path. Worker regressions cover narration rejection, quiet self-speak failure and ordinary asterisk emphasis. `npm run test:simulation` and the full `npm run check` remain the release gates.
...
- **Tap uses; drag moves; drag to inventory puts away.** Placed objects follow one physical model with no Use/Put away popup and no permanent toolbar: activation — pointer or keyboard — starts the canonical use flow in every shelf state, a drag (12 px threshold, no long-press) repositions, and dropping a dragged room object on the shelf target that appears only during the drag puts it away. With the shelf open each room object also shows an explicit, labelled Put away control that keeps per-object put-away keyboard- and screen-reader-accessible. The release decision is a pure classifier, so a drag can never fall through to Use.
